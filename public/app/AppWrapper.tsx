...
import { type ComponentType, Fragment, type ReactNode, useEffect, useMemo, useState } from 'react';
...
export const AppWrapper = ({ context }: AppWrapperProps) => {
  const [ready, setReady] = useState<boolean>(false);
  const [registries, setRegistries] = useState<PluginExtensionRegistries | undefined>(undefined);
  const iconCacheID = useMemo(() => `grafana-icon-cache-${config.buildInfo.commit}`, []);

  useEffect(() => {
    void (async () => {
      const loadedRegistries = await getPluginExtensionRegistries();
      setReady(true);
  ...
  const routerWrapperProps = {
    routes: ready && <Routes>{getAppRoutes().map((r) => renderRoute(r))}</Routes>,
    pageBanners,
    bodyRenderHooks,
