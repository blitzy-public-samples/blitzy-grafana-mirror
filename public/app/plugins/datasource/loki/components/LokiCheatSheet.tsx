...
const LokiCheatSheet = (props: QueryEditorHelpProps<LokiQuery>) => {
  const [userExamples, setUserExamples] = useState<string[]>([]);
  const userLabelTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  const styles = useStyles2(getStyles);

  useEffect(() => {
    const checkUserLabels = async () => {
      // Set example from user labels
      const provider: LokiLanguageProvider = props.datasource?.languageProvider;
      if (provider.started) {
        const labels = provider.getLabelKeys() || [];
        const preferredLabel = PREFERRED_LABELS.find((l) => labels.includes(l));
        if (preferredLabel) {
          const values = await provider.fetchLabelValues(preferredLabel);
          const newUserExamples = shuffle(values)
            .slice(0, EXAMPLES_LIMIT)
            .map((value) => `{${preferredLabel}="${escapeLabelValueInExactSelector(value)}"}`);
          setUserExamples(newUserExamples);
        }
      } else {
        userLabelTimerRef.current = setTimeout(checkUserLabels, 1000);
      }
    };
    ...
    return () => {
      if (userLabelTimerRef.current !== undefined) {
        clearTimeout(userLabelTimerRef.current);
      }
    };
  }, [props.datasource]);
