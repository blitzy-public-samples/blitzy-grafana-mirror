...
export const QueryRows = memo(function QueryRows(props: Props) {
  const { queries, expressions, condition, data, onQueriesChange, onDuplicateQuery, onRunQueries, onSetCondition } =
    props;

  // Stable identity: only depends on the module-level `getDataSourceSrv` import
  // and the `query` argument, so no dependencies are needed.
  const getDataSourceSettings = useCallback((query: AlertQuery): DataSourceInstanceSettings | undefined => {
  ...
  const onChangeTimeRange = useCallback(
    (timeRange: RelativeTimeRange, index: number) => {
      onQueriesChange(
  ...
  const onChangeQueryOptions = useCallback(
    (options: AlertQueryOptions, index: number) => {
      onQueriesChange(
        queries.map((item, itemIndex) => {
          if (itemIndex !== index) {
            return item;
  ...
  const onChangeDataSource = useCallback(
    (settings: DataSourceInstanceSettings, index: number) => {
    ...
    [queries, onQueriesChange, getDataSourceSettings]
  );
...
QueryRows.displayName = 'QueryRows';
