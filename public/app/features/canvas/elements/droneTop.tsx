...
const DroneTopDisplay = ({ data }: CanvasElementProps<DroneTopConfig, DroneTopData>) => {
  ...
  const droneTopTransformClass = useMemo(() => css({ transform: droneTopTransformStyle }), [droneTopTransformStyle]);

  const fRightRotorAnimationClass = useMemo(
    () =>
      css({
        // eslint-disable-next-line @grafana/no-unreduced-motion -- canvas drone rotor animation is essential to the visualization; the parent canvas panel surfaces RPM values via scalar dimension and motion is the intended user-facing rendering
        animation: fRightRotorAnimation,
      }),
    [fRightRotorAnimation]
  );

  const fLeftRotorAnimationClass = useMemo(
    () =>
      css({
        // eslint-disable-next-line @grafana/no-unreduced-motion -- canvas drone rotor animation is essential to the visualization; the parent canvas panel surfaces RPM values via scalar dimension and motion is the intended user-facing rendering
  ...
  const bRightRotorAnimationClass = useMemo(
    () =>
      css({
        // eslint-disable-next-line @grafana/no-unreduced-motion -- canvas drone rotor animation is essential to the visualization; the parent canvas panel surfaces RPM values via scalar dimension and motion is the intended user-facing rendering
        animation: bRightRotorAnimation,
      }),
    [bRightRotorAnimation]
  );

  const bLeftRotorAnimationClass = useMemo(
    () =>
      css({
        // eslint-disable-next-line @grafana/no-unreduced-motion -- canvas drone rotor animation is essential to the visualization; the parent canvas panel surfaces RPM values via scalar dimension and motion is the intended user-facing rendering
  ...
  return (
    <svg
      ...
      xmlSpace="preserve"
      className={cx(styles.droneTop, droneTopTransformClass)}
    >
      <path
        fillRule="evenodd"
      ...
      <g className="propeller-group">
        <path
          className={cx(styles.propeller, styles.propellerCW, bRightRotorAnimationClass)}
        ...
        />
        <path
          className={cx(styles.propeller, styles.propellerCCW, fRightRotorAnimationClass)}
        ...
        />
        <path
          className={cx(styles.propeller, styles.propellerCCW, bLeftRotorAnimationClass)}
        ...
        />
        <path
          className={cx(styles.propeller, styles.propellerCW, fLeftRotorAnimationClass)}
