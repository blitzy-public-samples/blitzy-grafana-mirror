...
/**
 * Checks whether the golden files output directory is present in the working tree.
 *
 * Background: golden files for dashboard migration parity tests are deliberately not tracked in git
 * (see `apps/dashboard/pkg/migration/.gitignore`). They are generated by the Go backend tests via
 * `make generate-golden-files` and uploaded as a CI artifact for frontend test shards to download.
 *
 * In CI the directory is populated before tests run. In frontend-only developer environments (and any
 * sandbox without Go), the directory is missing and the dependent suites must be skipped to avoid
 * spurious ENOENT failures that are unrelated to the code under test.
 *
 * Returns `true` when the requested `output/<outputType>` directory exists, `false` otherwise.
 */
export function hasGoldenFilesOutputDir(outputType: 'single_version' | 'latest_version'): boolean {
  return existsSync(getOutputDirectory(outputType));
}

/**
 * Returns the inputDir if it exists, otherwise null. Used by suites that need to bail at module
 * load time when the migration testdata input directory is missing.
 */
export function getInputDirIfPresent(): string | null {
  const { inputDir } = getTestDirectories();
  return existsSync(inputDir) ? inputDir : null;
}
