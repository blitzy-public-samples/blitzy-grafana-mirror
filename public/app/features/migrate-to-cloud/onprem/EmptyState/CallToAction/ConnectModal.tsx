...
import { Modal, Button, Form, Stack, TextLink, Field, Input, Text, useStyles2 } from '@grafana/ui';
...
export const ConnectModal = ({ isOpen, isLoading, error, hideModal, onConfirm }: Props) => {
  ...
  return (
    <Modal
      ...
      onDismiss={hideModal}
    >
      <Form<FormData> defaultValues={{ token: '' }} onSubmit={onConfirmConnect} maxWidth="none">
              ...
              <Text color="secondary">
                <Stack direction="column" gap={2}>
                  ...
                  <Trans i18nKey="migrate-to-cloud.connect-modal.body-cloud-stack">
                    You&apos;ll also need a cloud stack. If you just signed up, we&apos;ll automatically create your
                    first stack. If you have an account, you&apos;ll need to select or create a stack.
                  </Trans>
                  ...
                  <div>
                    <Trans i18nKey="migrate-to-cloud.connect-modal.body-token">
                      Your self-managed Grafana installation needs special access to securely migrate content.
                      You&apos;ll need to create a migration token on your chosen cloud stack.
                    </Trans>
                  </div>

                  <div>
                    <Trans i18nKey="migrate-to-cloud.connect-modal.body-token-instructions">
                      Log into your cloud stack and navigate to Administration &gt; General &gt; Migrate to Grafana
                      Cloud. Create a migration token on that screen and paste the token here.
                    </Trans>
                  </div>
                  ...
                  >
                    <Input
                      {...register('token', {
                        required: t(
                          'migrate-to-cloud.connect-modal.token-required-error',
